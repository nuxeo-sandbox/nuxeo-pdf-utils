...
/**
 * Encrypts the PDF to "readonly" for users. Read only means they can read, copy, and print. They can't assemble, modify,
 * modify annotations, ...
 * 
 * @since 8.1
 */
@Operation(id = PDFEncryptReadOnlyOp.ID, category = Constants.CAT_CONVERSION, label = "PDF: Encrypt Read Only", description = "Encrypts the pdf, returns a copy. User can read, print and copy but cannot modify. originalOwnerPwd is used if the pdf was originally encrypted. If ownerPwd is empty, use originalOwnerPwd to encrypt. If no keyLentgh is provided, use 128. If the operaiton is ran on Document(s), xpath lets you specificy where to get the blob from (default: file:content)")
public class PDFEncryptReadOnlyOp {
